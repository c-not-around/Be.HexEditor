...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		void PaintLineInfo(Graphics g, long startByte, long endByte)
		{
			...
			Color lineInfoColor = Enabled ? ((InfoForeColor != Color.Empty) ? InfoForeColor : ForeColor) : Color.Gray;
		...
		{
            Brush brush = new SolidBrush(Enabled ? HeaderForeColor : Color.Gray);

			for (int col = 0; col < _iHexMaxHBytes; col++)
			{
				PaintColumnInfo(g, (byte)col, new SolidBrush(InfoForeColor)/*brush*/, col);
			}
            
            g.DrawString("-Address-", Font, brush, new PointF(_recLineInfo.X, _recColumnInfo.Y), _stringFormat);

            float y = (_recColumnInfo.Y + _recColumnInfo.Height + GetBytePointF(new Point(0, 0)).Y) / 2.2f;
            g.DrawLine(new Pen(Color.DimGray), new PointF(_recLineInfo.X, y), new PointF(_recStringView.X + _recStringView.Width, y));

            float x = _recStringView.X + (_recStringView.Width - g.MeasureString("ASCII", Font).Width) / 2;
            g.DrawString("ASCII", Font, brush, new PointF(x, _recColumnInfo.Y), _stringFormat);
        }
		...
		void PaintHex(Graphics g, long startByte, long endByte)
		{
			...
			for (long i = startByte; i < intern_endByte + 1; i++)
			{
				...
				{
					PaintHexStringSelected(g, b, selBrush, selBrushBack, gridPoint, i == (_bytePos + _selectionLength - 1));
				}
				else
				{
		...
		void PaintHexStringSelected(Graphics g, byte b, Brush brush, Brush brushBack, Point gridPoint, bool last = false)
		{
            ...
            float bcWidth = (last || (gridPoint.X + 1 == _iHexMaxHBytes)) ? _charSize.Width * 2 : _charSize.Width * 3;
		...
		void PaintHexAndStringView(Graphics g, long startByte, long endByte)
		{
			...
			Brush selBrush = new SolidBrush(_selectionForeColor);
			Brush selBrushBack = new SolidBrush(_selectionBackColor);
            Brush asciiBrush = new SolidBrush(Enabled ? AsciiForeColor : Color.Gray);
			...
			for (long i = startByte; i < intern_endByte + 1; i++)
			{
				...
				if (isSelectedByte && isStringKeyInterpreterActive)
				{
                    ...
                    g.DrawString(s, Font, selBrush/*asciiBrush*/, byteStringPointF, _stringFormat);
				}
				else
				{
					g.DrawString(s, Font, asciiBrush, byteStringPointF, _stringFormat);
				}
			}
		}
		...
		public Color BackColorDisabled
		{
			...
			set
			{
                if (value != _backColorDisabled && value != Color.Empty)
                {
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("Appearance"), Description("Gets or sets the border color.")]
        public Color BorderColor
        {
            get => _borderColor;
            set
            {
                if (value != _borderColor && value != Color.Empty)
                {
                    _borderColor = value;
                    Invalidate();
                }
		...
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the line info color. When this property is null, then ForeColor property is used.")]
		public Color InfoForeColor
		{
			get => _infoForeColor;
			set 
            {
                if (value != _infoForeColor && value != Color.Empty)
                {
                    _infoForeColor = value;
                    Invalidate();
                }
            }
		} Color _infoForeColor = Color.Black;

        /// <summary>
		/// Gets or sets the ascii color. When this property is null, then ForeColor property is used.
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the ascii color. When this property is null, then ForeColor property is used.")]
        public Color AsciiForeColor
        {
            get => _asciiForeColor;
            set 
            {
                if (value != _asciiForeColor && value != Color.Empty)
                {
                    _asciiForeColor = value;
                    Invalidate();
                }
            }
        ...
        /// <summary>
		/// Gets or sets the header color. When this property is null, then ForeColor property is used.
		/// </summary>
		[DefaultValue(typeof(Color), "Black"), Category("Hex"), Description("Gets or sets the header color. When this property is null, then ForeColor property is used.")]
        public Color HeaderForeColor
        {
            get => _headerForeColor;
            set
            {
                if (value != _headerForeColor && value != Color.Empty)
                {
                    _headerForeColor = value;
                    Invalidate();
                }
            }
        } Color _headerForeColor = Color.Black;
		...
		public Color SelectionBackColor
		{
			...
			set
            {
                if (value != _selectionBackColor && value != Color.Empty)
                {
		...
		public Color SelectionForeColor
		{
			...
			set
            {
                if (value != _selectionForeColor && value != Color.Empty)
                {
		...
		public Color ShadowSelectionColor
		{
			...
			set
            {
                if (value != _shadowSelectionColor && value != Color.Empty)
                {
		...
		public IByteCharConverter ByteCharConverter
		{
			get
			{
                ...
                {
                    _byteCharConverter = new AnsiByteCharConvertor();
                }
